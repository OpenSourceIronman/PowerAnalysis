...
class Simulation:
    ...
    def run(self, runTimeInSeconds: int):
...
if __name__ == "__main__":
    motor = Consumption("Motor", 5, 0, 2, 5, 100)
    cpu = Consumption("CPU", 2, 0, 4, 8, 100)

    submodules = [motor, cpu]

    powerModes = [{motor: Consumption.AVG_POWER_DRAW_MODE,
                cpu: Consumption.AVG_POWER_DRAW_MODE}, 3600 * Simulation.ONE_SECOND]

    battery = BatteryCell(3.65, 18, 10, BatteryCell.LI_FE_P_O4)
    ...
    simulation.run(3599 * Simulation.ONE_SECOND)
